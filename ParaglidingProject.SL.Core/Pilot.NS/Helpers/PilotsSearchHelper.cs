            ...
            switch (options.SearchBy)
            {
                ...
                case PilotsSearches.FirstName:
                    return pilots
                        ...
                        .Where(p => pilots.Any(pilots => p.FirstName.Contains(options.UserInput)));

                case PilotsSearches.LastName:
                    return pilots
                        //.Where(p => p.LastName.Contains(options.UserInput));
                        .Where(p => pilots.Any(pilots => p.LastName.Contains(options.UserInput)));

                case PilotsSearches.Address:
                    return pilots
                        //.Where(p => p.Address.Contains(options.UserInput));
                        .Where(p => pilots.Any(pilots => p.Address.Contains(options.UserInput)));

                case PilotsSearches.PhoneNumber:
                    return pilots
                        .Where(p => pilots.Any(pilots => p.PhoneNumber.Contains(options.UserInput)));
